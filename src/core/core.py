...
from src.core import abuse_detected
...
def sanitize_username(username: str, telegram_id: int=None) -> str:
    import re
    suffix = telegram_id if telegram_id is not None else 'mail'
    if not username:
...
def create_user_and_subscription(telegram_id: int=None, username: str='', days: int=None, referred_by: int=None, traffic_limit: int=None, squad_uuids: list=None, plan_type: str='vpn', devices_limit: int=2, force_new: bool=False, expire_at: datetime=None, email: str=None, existing_user_id: int=None) -> Optional[Dict]:
    try:
        ...
        if existing_user_id:
            ...
            user_id = database.create_user(telegram_id, username, referred_by=referred_by, email=email)
        tid_for_rw = telegram_id if telegram_id is not None else (1000000000 + int(user_id or 0))
                ...
                if not squad_uuids:
                    ...
                    if not squad_uuids:
                        ...
                        try:
                            ...
                            conn.close()
        logger.info(f'Creating subscription for {telegram_id or email or user_id}, plan_type={plan_type}, squads={squad_uuids}, expire_at={expire_at.isoformat()}')
        remnawave_user = None
        existing_users = remnawave.remnawave_api.get_user_by_telegram_id(telegram_id) if telegram_id is not None else []
        if not force_new and existing_users and (len(existing_users) > 0):
            ...
            if existing_uuid:
                ...
                updated_user = remnawave.remnawave_api.update_user_sync(uuid=existing_uuid, expire_at=expire_at, traffic_limit_bytes=traffic_limit or 0, hwid_device_limit=devices_limit, active_internal_squads=squad_uuids if squad_uuids else None, email=email)
                if updated_user:
        ...
        if not remnawave_user:
            ...
            base_username = sanitize_username(username, tid_for_rw)
            unique_username = f'{base_username}_{timestamp}'
            try:
                remnawave_user = remnawave.remnawave_api.create_user_with_params(telegram_id=telegram_id, username=unique_username, days=days, traffic_limit_bytes=traffic_limit or 0, hwid_device_limit=devices_limit, active_internal_squads=squad_uuids if squad_uuids else None, expire_at=expire_at, email=email)
            except Exception as create_error:
                error_msg = str(create_error).lower()
                if 'already exists' in error_msg or 'a019' in error_msg:
                    import random
                    unique_username = f'{base_username}_{tid_for_rw}_{random.randint(1000, 9999)}'
                    logger.info(f'Username collision, trying {unique_username}')
                    remnawave_user = remnawave.remnawave_api.create_user_with_params(telegram_id=telegram_id, username=unique_username, days=days, traffic_limit_bytes=traffic_limit or 0, hwid_device_limit=devices_limit, active_internal_squads=squad_uuids if squad_uuids else None, expire_at=expire_at, email=email)
                else:
                    raise create_error
        if not remnawave_user:
            logger.error(f'Failed to create user in Remnawave: {telegram_id or email}')
            return None
        if hasattr(remnawave_user, 'uuid'):
